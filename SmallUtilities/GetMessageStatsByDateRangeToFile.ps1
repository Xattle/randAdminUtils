<#
.NOTES
Scavenged by Xattle from the wayback machine
...
By default does not include messages for the Exchange 2013 Health Mailboxes, use the Include2013HealthMailboxes to include them in report.
.PARAMETER StartDate
Specifies the start date to go back and search through the Message Tracking Logs. Expected in mm/dd/yyyy format.
.PARAMETER EndDate
Specifies the end date to go back and search through the Message Tracking Logs.  Expected in mm/dd/yyyy format.
...
.EXAMPLE
PS> .\GetMessageStatsToFile.ps1 -StartDate 30
...
.EXAMPLE
PS> .\GetMessageStatsToFile.ps1 -StartDate 15 -IncludeDistListStats
...
Param(
    ...
    HelpMessage="Enter the start date in mm/dd/yyyy format.")]
    $StartDate,

    [Parameter(Position=1, Mandatory = $true,
      HelpMessage="Enter the end date in mm/dd/yyyy format.")]
      $EndDate,
...
$outfile_end_date = ([datetime]$end_date).tostring("yyyy_MM_dd")
$outfile_start_date = ([datetime]$start_date).tostring("yyyy_MM_dd")
$outfile = "DaysMessageStats_"+ $outfile_start_date + " - " + $outfile_end_date + ".csv"

$dloutfile = "DistListStats_" + $outfile_end_date +".csv"
...
@"
        ...
        Sent Total = $(0 + $total_msgsent[$address] + $unique_msgsent_exch[$address])
        ...
        Sent Internal = $(0 + $total_msgsent_exch[$address] + $unique_msgsent_exch[$address])
...
foreach ($ht in $hts){
    ...
    get-messagetrackinglog -Server $ht -Start "$start_date" -End "$end_date 11:59:59 PM" -resultsize unlimited |
    time_pipeline |%{
...
If ($IncludeDistListStats) {
        ...
        $DL_stats | Foreach {
            if ($dl[$_.address]){
                if ([datetime]$_.lastused -le [datetime]$end_date){
        ...
        $dl.keys | Foreach {
            if ($dl_list -notcontains $_){
                ...
                $new_rec.address = $_
                $new_rec.used = $dl[$_]
                $new_rec.Since = $end_date
                $new_rec.lastused = $end_date
                $dl_stats += @($new_rec)
            }
        }
