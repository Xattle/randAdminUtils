...
## Uncomment below to scan everything
$computers = Get-ADComputer -Filter 'Name -like "*"'

$output = @()

foreach ($computer in $computers) {
    $result = Get-LoggedInUser -ComputerName $computer.name
    $output += $result
}
...
$logoffsessions = $($result | Out-GridView -PassThru)

foreach ($usersession in $logoffsessions) {
    Invoke-RDUserLogoff -HostServer $usersession.ComputerName -UnifiedSessionID $usersession.id -Force
}
